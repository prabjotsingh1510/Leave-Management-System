...
try {
    ...
    if ($auth_data['success']) {
        ...
        $stmt = $pdo->prepare("SELECT wa.id, wa.reg_no, wa.place_of_visit, wa.date, wa.purpose_of_visit, wa.time_slot, wa.status, s.name AS student_name, sh.hostel_block
        FROM weekend_applications wa
        INNER JOIN student_hostel sh ON wa.reg_no = sh.reg_no
        INNER JOIN students s ON wa.reg_no = s.reg_no
        INNER JOIN wardens w ON w.block = sh.hostel_block
        WHERE w.warden_id = :warden_id
        AND wa.status = 'MApproved'");
