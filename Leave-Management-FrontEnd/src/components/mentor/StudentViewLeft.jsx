...
import { AxiosPost } from '../../context/AuthContext'
...
const StudentViewLeft = (props) => {
  ...
  const fetchStudent = useCallback(async () => {
    setIsLoading(true);
    try {
      const data = await AxiosPost("fetchStudent.php", {
        reg_no: props.selectedStudent,
      });
      if (data.success) {
        setData(data.data);
  ...
  }, [fetchStudent, props.selectedStudent]);
  ...
  if (data) {
    return (
      <Grid container spacing={6}>
        <Grid item xs={12}>
          <Card>
            <Button size='small' color='secondary' sx={{ m: 3, mb: 0 }} variant="contained" onClick={() => props.setSelectedStudent(null)}>
              Go back
            </Button>
            ...
            <CardContent>
              ...
              <Box sx={{ pt: 2, pb: 1 }}>
                <Box sx={{ display: 'flex', mb: 2.7 }}>
                  ...
                  <Typography variant='body2'>@{data.reg_no}</Typography>
                </Box>
                <Box sx={{ display: 'flex', mb: 2.7 }}>
                  <Typography variant='subtitle2' sx={{ mr: 2, color: 'text.primary' }}>
                    Email:
                ...
                <Box sx={{ display: 'flex', mb: 2.7 }}>
                  ...
                  <CustomChip
                    skin='light'
                    size='small'
                    label={data.application_number}
                    color={statusColors[data.application_number]}
                    sx={{
                      height: 20,
                      fontWeight: 500,
                      fontSize: '0.75rem',
                ...
                <Box sx={{ display: 'flex', mb: 2.7 }}>
                  <Typography sx={{ mr: 2, fontWeight: 500, fontSize: '0.875rem' }}>Batch:</Typography>
                ...
                <Box sx={{ display: 'flex', mb: 2.7 }}>
                  <Typography sx={{ mr: 2, fontWeight: 500, fontSize: '0.875rem' }}>Parents Contact:</Typography>
                  <Typography variant='body2'>{data.parent_contact}</Typography>
                </Box>
                <Box sx={{ display: 'flex', mb: 2.7 }}>
                  <Typography sx={{ mr: 2, fontWeight: 500, fontSize: '0.875rem' }}>Hostel Info:</Typography>
                  <Typography variant='body2'>{data.hostel_block} - {data.room_number} - {data.room_type}</Typography>
                </Box>
              </Box>
            </CardContent>
