...
const WardenGeneralApplications = ({ setSelectedStudent }) => {
    const columns = [
        {
            flex: 0.2,
            minWidth: 230,
                ...
                return (
                    <Box sx={{ display: 'flex', alignItems: 'center' }}>
                        <Badge overlap='circular'>
                            <Avatar alt='John Doe' src={'https://ui-avatars.com/api/?name=' + row.student_name + '&background=random'} sx={{ width: '2.5rem', height: '2.5rem', mr: 2 }} />
                        </Badge>
                        <Box sx={{ display: 'flex', alignItems: 'flex-start', flexDirection: 'column' }}>
                        <LinkStyled href="#" onClick={((e) => { setSelectedStudent(row.reg_no); })}>{row.student_name}</LinkStyled>
        ...
        },
        {
            ...
            renderCell: ({ row }) => {
                if (row.status === 'MApproved') {
                    ...
                    return (
                        <Typography noWrap variant='body2' color='red'>
    ...
    const [generalApplicationsWarden, setGeneralApplicationsWarden] = useState();
    const { user } = useContext(AuthContext);

    const fetchGeneralApplicationsWarden = async () => {
        setIsLoading(true);
        try {
            const data = await AxiosPost('fetchGeneralApplicationsWarden.php', { warden_id: user.reg_no })
            // console.log(data)
            if (data.success) {
                setGeneralApplicationsWarden(data.weekend_applications)
            }
    ...
    const handleOutingAccepted = async () => {
        try {
            const data = await AxiosPost('acceptedByWardenGeneral.php', { id: selectedRecord.id })
            // console.log(data)
            if (data.success) {
                await fetchGeneralApplicationsWarden();
    ...
    useEffect(() => {
        ...
        fetchGeneralApplicationsWarden()
    }, [])


    if (!generalApplicationsWarden) {
        return (
            <div style={{
                display: 'flex',
        ...
        return (
            <>
                <DataGrid
                    rows={generalApplicationsWarden}
                    autoHeight
                    columns={columns}
                    loading={isLoading}
                    getRowId={getRowId}
                    ...
                    >
                        ...
                        >
                            ...
                            <Box sx={{ mb: 3 }}>
                                ...
                                <Typography>From Date: {selectedRecord && selectedRecord.from_date}</Typography>
                                <Typography>To Date: {selectedRecord && selectedRecord.to_date}</Typography>
...
export default WardenGeneralApplications;
